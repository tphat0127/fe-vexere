<template>
  <a-form
    ref="ruleStationForm"
    :model="createStationForm"
    :rules="rules"
    :label-col="labelCol"
    :wrapper-col="wrapperCol"
  >
    <a-form-item ref="name" label="Station name" name="name">
      <a-input v-model:value="createStationForm.name" />
    </a-form-item>
    <a-form-item ref="province" label="Province" name="province">
      <a-select v-model:value="createStationForm.province" style="width: 120px">
        <a-select-option
          v-for="pro in provinceList"
          :key="pro"
          :value="pro.state_name"
        >
          {{ pro.state_name }}
        </a-select-option>
      </a-select>
    </a-form-item>
    <a-form-item ref="address" label="Address" name="address">
      <a-input v-model:value="createStationForm.address" />
    </a-form-item>
    <a-form-item :wrapper-col="{ span: 14, offset: 4 }">
      <a-button
        type="primary"
        @click="onSubmit"
        :loading="$store.state.modules.loading"
      >
        Create
      </a-button>
      <a-button style="margin-left: 10px;" @click="resetForm">
        Reset
      </a-button>
    </a-form-item>
  </a-form>
</template>
<script>
export default {
  data() {
    return {
      provinceList: [
        {
          state_code: "An Giang",
          state_name: "An Giang"
        },
        {
          state_code: "Bà Rịa–Vũng Tàu",
          state_name: "Bà Rịa–Vũng Tàu"
        },
        {
          state_code: "Bình Dương",
          state_name: "Bình Dương"
        },
        {
          state_code: "Bình Phước",
          state_name: "Bình Phước"
        },
        {
          state_code: "Bình Thuận",
          state_name: "Bình Thuận"
        },
        {
          state_code: "Bình Định",
          state_name: "Bình Định"
        },
        {
          state_code: "Bạc Liêu",
          state_name: "Bạc Liêu"
        },
        {
          state_code: "Bắc Giang",
          state_name: "Bắc Giang"
        },
        {
          state_code: "Bắc Kạn",
          state_name: "Bắc Kạn"
        },
        {
          state_code: "Bắc Ninh",
          state_name: "Bắc Ninh"
        },
        {
          state_code: "Bến Tre",
          state_name: "Bến Tre"
        },
        {
          state_code: "Cao Bằng",
          state_name: "Cao Bằng"
        },
        {
          state_code: "Cà Mau",
          state_name: "Cà Mau"
        },
        {
          state_code: "Cần Thơ",
          state_name: "Cần Thơ"
        },
        {
          state_code: "Gia Lai",
          state_name: "Gia Lai"
        },
        {
          state_code: "Hà Giang",
          state_name: "Hà Giang"
        },
        {
          state_code: "Hà Nam",
          state_name: "Hà Nam"
        },
        {
          state_code: "Hà Nội",
          state_name: "Hà Nội"
        },
        {
          state_code: "Hà Tĩnh",
          state_name: "Hà Tĩnh"
        },
        {
          state_code: "Hòa Bình",
          state_name: "Hòa Bình"
        },
        {
          state_code: "Hưng Yên",
          state_name: "Hưng Yên"
        },
        {
          state_code: "Hải Dương",
          state_name: "Hải Dương"
        },
        {
          state_code: "Hải Phòng",
          state_name: "Hải Phòng"
        },
        {
          state_code: "Hậu Giang",
          state_name: "Hậu Giang"
        },
        {
          state_code: "Hồ Chí Minh",
          state_name: "Hồ Chí Minh"
        },
        {
          state_code: "Khánh Hòa",
          state_name: "Khánh Hòa"
        },
        {
          state_code: "Kiên Giang",
          state_name: "Kiên Giang"
        },
        {
          state_code: "Kon Tum",
          state_name: "Kon Tum"
        },
        {
          state_code: "Lai Châu",
          state_name: "Lai Châu"
        },
        {
          state_code: "Long An",
          state_name: "Long An"
        },
        {
          state_code: "Lào Cai",
          state_name: "Lào Cai"
        },
        {
          state_code: "Lâm Đồng",
          state_name: "Lâm Đồng"
        },
        {
          state_code: "Lạng Sơn",
          state_name: "Lạng Sơn"
        },
        {
          state_code: "Nam Định",
          state_name: "Nam Định"
        },
        {
          state_code: "Nghệ An",
          state_name: "Nghệ An"
        },
        {
          state_code: "Ninh Bình",
          state_name: "Ninh Bình"
        },
        {
          state_code: "Ninh Thuận",
          state_name: "Ninh Thuận"
        },
        {
          state_code: "Phú Thọ",
          state_name: "Phú Thọ"
        },
        {
          state_code: "Phú Yên",
          state_name: "Phú Yên"
        },
        {
          state_code: "Quảng Bình",
          state_name: "Quảng Bình"
        },
        {
          state_code: "Quảng Nam",
          state_name: "Quảng Nam"
        },
        {
          state_code: "Quảng Ngãi",
          state_name: "Quảng Ngãi"
        },
        {
          state_code: "Quảng Ninh",
          state_name: "Quảng Ninh"
        },
        {
          state_code: "Quảng Trị",
          state_name: "Quảng Trị"
        },
        {
          state_code: "Sóc Trăng",
          state_name: "Sóc Trăng"
        },
        {
          state_code: "Sơn La",
          state_name: "Sơn La"
        },
        {
          state_code: "Thanh Hóa",
          state_name: "Thanh Hóa"
        },
        {
          state_code: "Thái Bình",
          state_name: "Thái Bình"
        },
        {
          state_code: "Thái Nguyên",
          state_name: "Thái Nguyên"
        },
        {
          state_code: "Thừa Thiên–Huế",
          state_name: "Thừa Thiên–Huế"
        },
        {
          state_code: "Tiền Giang",
          state_name: "Tiền Giang"
        },
        {
          state_code: "Trà Vinh",
          state_name: "Trà Vinh"
        },
        {
          state_code: "Tuyên Quang",
          state_name: "Tuyên Quang"
        },
        {
          state_code: "Tây Ninh",
          state_name: "Tây Ninh"
        },
        {
          state_code: "Vĩnh Long",
          state_name: "Vĩnh Long"
        },
        {
          state_code: "Vĩnh Phúc",
          state_name: "Vĩnh Phúc"
        },
        {
          state_code: "Yên Bái",
          state_name: "Yên Bái"
        },
        {
          state_code: "Điện Biên",
          state_name: "Điện Biên"
        },
        {
          state_code: "Đà Nẵng",
          state_name: "Đà Nẵng"
        },
        {
          state_code: "Đắk Lắk",
          state_name: "Đắk Lắk"
        },
        {
          state_code: "Đắk Nông",
          state_name: "Đắk Nông"
        },
        {
          state_code: "Đồng Nai",
          state_name: "Đồng Nai"
        },
        {
          state_code: "Đồng Tháp",
          state_name: "Đồng Tháp"
        }
      ],
      labelCol: { span: 4 },
      wrapperCol: { span: 14 },
      createStationForm: {
        name: "",
        province: "",
        address: ""
      },
      rules: {
        name: [
          {
            required: true,
            message: "Please input station name",
            trigger: "blur"
          }
        ],
        address: [
          { required: true, message: "Please input address", trigger: "blur" }
        ]
      }
    };
  },
  methods: {
    onSubmit() {
      this.$refs.ruleStationForm
        .validate()
        .then(() => {
          this.$store.dispatch("actFetchCreateStation", this.createStationForm);
        })
        .catch(error => {
          console.log("error", error);
        });
    },
    resetForm() {
      this.$refs.ruleStationForm.resetFields();
    }
  }
};
</script>
